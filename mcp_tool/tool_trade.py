...
class PositionLock:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.lock_fd.close()
            raise RuntimeError("无法获取持仓文件锁，可能有其他进程正在交易")
...
def buy(symbol: str, amount: int) -> Dict[str, Any]:
    ...
    with _position_lock(signature):
        try:
            ...
            return {
                "error": f"Failed to load latest position: {e}",
                "symbol": symbol,
                "date": today_date,
            }
    ...
    try:
        this_symbol_price = get_open_prices(today_date, [symbol], market=market)[
            f"{symbol}_price"
        ]
    except KeyError:
    ...
    try:
        costs = calculate_trade_cost(symbol, this_symbol_price, amount, "buy")
        total_cost = this_symbol_price * amount + costs["total_cost"]
        ...
        for line in f:
            ...
            try:
                ...
                if record.get("date") == today_date:
                    ...
                    if (
                        this_action.get("action") == "buy"
                        and this_action.get("symbol") == symbol
                    ):
...
def sell(symbol: str, amount: int) -> Dict[str, Any]:
    ...
    costs = calculate_trade_cost(symbol, this_symbol_price, amount, "sell")
    net_proceeds = this_symbol_price * amount - costs["total_cost"]
